...
def recognise_frame(
    ...
    model: str = "hog",
    max_faces_to_match: Optional[int] = None,
) -> list[MatchResult]:
    """
    ...
    max_faces_to_match : int | None
        Limit how many detected faces to encode/match per frame. Largest faces
        are matched first when a limit is provided.
    ...
    locations_to_process = orig_locations
    if max_faces_to_match is not None and max_faces_to_match > 0 and len(orig_locations) > max_faces_to_match:
        indexed_locations = list(enumerate(orig_locations))

        def _face_area(loc: tuple[int, int, int, int]) -> int:
            top, right, bottom, left = loc
            return max(0, bottom - top) * max(0, right - left)

        indexed_locations.sort(key=lambda item: _face_area(item[1]), reverse=True)
        selected = indexed_locations[:max_faces_to_match]
        selected.sort(key=lambda item: item[0])
        locations_to_process = [loc for _, loc in selected]

    # Compute encodings at original resolution for accuracy
    encodings = face_recognition.face_encodings(rgb, known_face_locations=locations_to_process)

    known_encodings, known_names = db.get_all()
    results: list[MatchResult] = []

    for enc, loc in zip(encodings, locations_to_process):
